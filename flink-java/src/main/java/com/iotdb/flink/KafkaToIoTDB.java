...
public class KafkaToIoTDB {
	...
	DataStream<EvaluationResult> evaluated =
        ...
        evaluated
	    .map(new RichMapFunction<EvaluationResult, SensorData>() {
		    @Override
		    public SensorData map(EvaluationResult result) {
	...
	evaluated
	    .flatMap(new FlatMapFunction<EvaluationResult, DLQRecord>() {

		    @Override
		    public void flatMap(
					EvaluationResult result,
					Collector<DLQRecord> out) {
			...
			if (result.context_anomaly != null) {
			    ...
			    System.out.println(
					       "[DLQ] Anomaly: " + dlq.anomaly_id +
					       " (" + dlq.anomaly_type + ") for " +
					       dlq.device_id);
			}
		    }
		})
